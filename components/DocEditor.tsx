...
import { PageNumberModal } from './doc/modals/PageNumberModal';
...
export const DocEditor: React.FC<Props> = ({ 
  ...
  // Estado de Página Atual (Core do Slide Mode)
  ...
  const { nextPage, prevPage } = useSlideNavigation({
    ...
    onPageChange: (newPage) => {
      ...
      // Reset scroll position for the container when changing slides
  ...
  // Translate calculation for Slide Mode
  const effectivePageHeight = pageLayout.currentPaper.heightPx + 20; 
  ...
  return (
    <div className={`flex flex-col h-full bg-bg relative overflow-hidden text-text ${modes.focus ? 'focus-mode' : ''}`}>
       {/* Top Bar */}
       <div className="bg-surface border-b border-border z-50 shrink-0 shadow-sm relative">
       ...
       <div className="flex-1 overflow-hidden relative flex bg-black">
          ...
          <div 
            ref={docScrollerRef} 
            className="flex-1 flex justify-center px-12 pb-12 relative overflow-y-auto items-start pt-8 cursor-default"
          >
             {/* Floating Controls */}
             ...
             <div 
                className="relative mb-8 transition-transform origin-top will-change-transform bg-transparent" 
                style={{ 
                ...
                >
                    ...
                    <div className="absolute inset-0 pointer-events-none z-0">
                        {pageLayout.pages.map((page, i) => {
                            ...
                            const pageNumConf = pageLayout.pageSettings.pageNumber;
                            
                            // Lógica de "Começar a mostrar a partir da página X"
                            // A página FÍSICA é 'i + 1'. O número a exibir depende do startAt.
                            // Mas a visibilidade depende do displayFromPage.
                            const physicalPage = i + 1;
                            const showPageNum = pageNumConf?.enabled && physicalPage >= (pageNumConf.displayFromPage || 1);
                            
                            // O valor numérico exibido (Ex: na página 3, se startAt=1, mostra 3)
                            // Se startAt fosse reiniciado, seria: physicalPage - (pageNumConf.displayFromPage || 1) + pageNumConf.startAt
                            // Mas ABNT geralmente conta tudo, só esconde. Então o valor é physicalPage.
                            // A prop startAt serve para casos onde a numeração começa em outro valor globalmente (ex: volume 2)
                            const pageNumValue = i + (pageNumConf?.startAt || 1);

                            return (
                                <div key={i} className="absolute left-0 w-full" style={{ top: i * effectivePageHeight, height: page.heightPx }}>
                                    {pageLayout.showRuler && (
                                    ...
                                    <div 
                                        className={`bg-white shadow-lg w-full h-full border border-[#333] flex flex-col justify-between relative`} 
                                    ...
                                    >
                                        ...
                                        <div className="relative">
                                            ...
                                            {/* Page Number (Header Position) */}
                                            {showPageNum && pageNumConf?.position === 'header' && (
                                                <div 
                                                    className="absolute top-6 px-12 w-full text-sm text-gray-500 pointer-events-none"
                                                    style={{ textAlign: pageNumConf.alignment }}
                                                >
                                                    {pageNumValue}
                                                </div>
                                            )}
                                        ...
                                        <div className="relative mt-auto">
                                            ...
                                            {showPageNum && pageNumConf?.position === 'footer' && (
                                                <div 
                                                    className="absolute bottom-6 px-12 w-full text-sm text-gray-500 pointer-events-none"
                                                    style={{ textAlign: pageNumConf.alignment }}
                                                >
                                                    {pageNumValue}
                                                </div>
                    ...
                    <div 
                        ref={contentRef} 
                        className="relative z-10 min-h-screen gpu-layer" 
                        style={{ 
       ...
       <PageNumberModal isOpen={modals.pageNumber} onClose={() => toggleModal('pageNumber', false)} onApply={(config) => { pageLayout.setPageSettings(prev => ({...prev, pageNumber: config})); toggleModal('pageNumber', false); }} />
       ...
       <HeaderFooterModal isOpen={modals.headerFooter} onClose={() => toggleModal('headerFooter', false)} initialHeader={pageLayout.pageSettings.headerText} initialFooter={pageLayout.pageSettings.footerText} onApply={(h, f) => pageLayout.setPageSettings(prev => ({...prev, headerText: h, footerText: f}))} activeTab={activeHeaderFooterTab} />
