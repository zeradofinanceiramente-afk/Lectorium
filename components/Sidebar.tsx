...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  useEffect(() => {
    let active = true;
    getOfflineCacheSize().then(size => { if (active && size) { setDownloadSize(size); setCachingStatus('done'); }});
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 left-0 z-50 bg-sidebar border-r border-border transition-transform duration-300 ease-in-out shadow-2xl flex flex-col w-72 ${isOpen ? 'translate-x-0' : '-translate-x-full'} print:hidden`}>
        ...
        <nav className="flex-1 py-6 space-y-2 overflow-y-auto custom-scrollbar overflow-x-hidden">
          <div className="space-y-1 px-3">
            ...
            <button onClick={() => handleNavigation('browser')} className={`w-full group p-3 rounded-xl transition-all duration-200 flex items-center px-4 ${activeTab === 'browser' || activeTab === 'offline' ? 'bg-brand/10 text-brand font-medium' : 'text-text-sec hover:bg-white/5 hover:text-text'}`}>
              <FolderOpen size={24} className={`shrink-0 ${activeTab === 'browser' ? "fill-brand/20" : ""}`} /><span className="ml-4 text-base">Arquivos</span>
            </button>
            <button onClick={() => handleNavigation('mindmaps')} className={`w-full group p-3 rounded-xl transition-all duration-200 flex items-center px-4 ${activeTab === 'mindmaps' ? 'bg-brand/10 text-brand font-medium' : 'text-text-sec hover:bg-white/5 hover:text-text'}`}>
              <Workflow size={24} className={`shrink-0 ${activeTab === 'mindmaps' ? "text-brand" : ""}`} /><span className="ml-4 text-base">Mapas Mentais</span>
            </button>
          </div>

          <div className="pt-4 px-3 border-t border-border">
            <button onClick={() => setIsThemesOpen(!isThemesOpen)} className="w-full group p-3 rounded-xl transition-all duration-200 flex items-center px-4 text-text-sec hover:bg-white/5 hover:text-text">
              <Palette size={24} className="shrink-0" />
              <div className="flex items-center justify-between flex-1 ml-4"><span className="text-sm">Temas</span>{isThemesOpen ? <ChevronDown size={14}/> : <ChevronRight size={14}/>}</div>
            </button>
            {isThemesOpen && (
      ...
      <BaseModal isOpen={showCustomizeModal} onClose={() => setShowCustomizeModal(false)} title="Personalizar Dashboard" icon={<Palette size={20} />} maxWidth="max-w-md">
        <div className="space-y-6">
            <p className="text-xs text-text-sec leading-relaxed">Escolha as imagens de fundo para o seu Dashboard.</p>
            <div className="grid grid-cols-1 gap-4">
                <div className="bg-bg p-4 rounded-2xl border border-border">
                    <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><Monitor size={16} className="text-brand" /><span className="text-sm font-bold">Desktop</span></div>{wallpapers.landscape && <button onClick={() => handleRemoveWallpaper('landscape')} className="text-red-400 p-1.5 hover:bg-red-500/10 rounded-lg transition-colors"><Trash2 size={14} /></button>}</div>
                    {wallpapers.landscape ? <div className="aspect-video w-full rounded-lg overflow-hidden border border-border relative group"><img src={wallpapers.landscape} className="w-full h-full object-cover" alt="Preview" /><label className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer"><span className="text-xs font-bold text-white flex items-center gap-2"><RefreshCw size={14} /> Alterar</span><input type="file" className="hidden" accept="image/*" onChange={(e) => handleWallpaperUpload(e, 'landscape')} /></label></div> : <label className="aspect-video w-full rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-white/5 transition-all"><Upload size={20} className="text-text-sec" /><span className="text-xs text-text-sec">Upload Horizontal</span><input type="file" className="hidden" accept="image/*" onChange={(e) => handleWallpaperUpload(e, 'landscape')} /></label>}
                </div>
                <div className="bg-bg p-4 rounded-2xl border border-border">
                    <div className="flex items-center justify-between mb-4"><div className="flex items-center gap-2"><Smartphone size={16} className="text-blue-400" /><span className="text-sm font-bold">Mobile</span></div>{wallpapers.portrait && <button onClick={() => handleRemoveWallpaper('portrait')} className="text-red-400 p-1.5 hover:bg-red-500/10 rounded-lg transition-colors"><Trash2 size={14} /></button>}</div>
                    {wallpapers.portrait ? <div className="aspect-[3/4] w-32 mx-auto rounded-lg overflow-hidden border border-border relative group"><img src={wallpapers.portrait} className="w-full h-full object-cover" alt="Preview" /><label className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer"><RefreshCw size={14} className="text-white" /><input type="file" className="hidden" accept="image/*" onChange={(e) => handleWallpaperUpload(e, 'portrait')} /></label></div> : <label className="aspect-[3/4] w-32 mx-auto rounded-lg border-2 border-dashed border-border flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-white/5 transition-all"><Upload size={20} className="text-text-sec" /><span className="text-xs text-text-sec text-center px-2">Upload Vertical</span><input type="file" className="hidden" accept="image/*" onChange={(e) => handleWallpaperUpload(e, 'portrait')} /></label>}
                </div>
            </div>
            <button onClick={() => setShowCustomizeModal(false)} className="w-full py-3 bg-surface border border-border text-text font-bold rounded-xl hover:bg-white/5 transition-colors">Fechar</button>
        </div>
      </BaseModal>
    </>
  );
